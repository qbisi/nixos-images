...
#define SGM37604A_MIN_VALUE_SETTINGS 16
#define SGM37604A_MAX_VALUE_SETTINGS 4095
#define MIN_MAX_SCALE(x) \
	(((x) < SGM37604A_MIN_VALUE_SETTINGS) ? SGM37604A_MIN_VALUE_SETTINGS : \
...
static char SGM37604A_i2c_write(struct i2c_client *client, u8 reg_addr, u8 *data, u8 len)
{
	...
	while (len-- != 0) {
		pr_debug("[SGM37604A] I2C Write: Reg=0x%02X, Data=0x%02X\n", reg_addr, *data);
...
static int SGM37604A_i2c_read(struct i2c_client *client, u8 reg_addr, u8 *data)
{
	s32 ret;
				      ...
				      unsigned char reg_addr, unsigned char *data)
{
	...
	for (i = 0; i < 5; i++) {
		if (ret_val != 0) {
			...
			ret_val = SGM37604A_i2c_read(client, 0x1F, &fault_reg);
			if (ret_val == 0) {
				pr_debug("[SGM37604A] Fault Reg(0x1F)=0x%02X: ", fault_reg);
				if (fault_reg & (1 << 4))
					pr_cont("SO(Open) ");
				if (fault_reg & (1 << 3))
					pr_cont("SC(Short) ");
				if (fault_reg & (1 << 2))
					pr_cont("TSD(OverTemp) ");
				if (fault_reg & (1 << 1))
					pr_cont("OC(OverCurrent) ");
				if (fault_reg & (1 << 0))
					pr_cont("OVP(OverVoltage) ");
				if (fault_reg == 0)
					pr_cont("No Fault");
				pr_cont("\n");
			}
...
void SGM37604A_set_backlight_reg_init(void)
{
	...
	data0 = 0x00;
	data1 = pchip->led_channels;
	data2 = pchip->max_current;

	gpiod_set_value(pchip->en_gpio, 1);
	...
	SGM37604A_set_brightness_level(0x75B);
}
...
{
	pr_debug("[SGM37604A] bl->props.brightness[%d]\n", bl->props.brightness);

	return bl->props.brightness;
}

static void sgm37604a_bled_enable(void)
{
...
static void sgm37604a_bled_disable(void)
{
	...
	msleep(2);
	gpiod_set_value(pchip->en_gpio, 0);
}
...
static int SGM37604A_probe(struct i2c_client *client)
{
...

	{
		...
		rc = of_property_read_u32(client->dev.of_node, "sgmicro,led-channels", &val32);
		if (rc) {
			pchip->led_channels = 0x07;
			pr_debug("[SGM37604A] led-channels not found in DT, using default 0x07 (LED1+LED2+DEV_EN)\n");
		} else {
			pchip->led_channels = (u8)val32;
		}
	}

	{
		...
		rc = of_property_read_u32(client->dev.of_node, "sgmicro,max-current", &current_val);
		if (rc) {
			pchip->max_current = 0x00;
		} else {
			if (current_val > 0x03)
				pchip->max_current = 0x00;
			else
				pchip->max_current = (u8)current_val;
		}
	}

	pchip->en_gpio = devm_gpiod_get_optional(&client->dev, "enable", GPIOD_OUT_LOW);
	if (IS_ERR(pchip->en_gpio)) {
		rc = PTR_ERR(pchip->en_gpio);
		pr_debug("[SGM37604A] failed to get enable gpio, rc=%d\n", rc);
		return rc;
	}

	if (pchip->en_gpio) {
		gpiod_set_value(pchip->en_gpio, 1);
		pr_debug("[SGM37604A] enable gpio configured\n");
	}
	...
	pchip->bled =
		devm_backlight_device_register(&client->dev, "sgm37604a",
					       &client->dev, pchip, &sgm37604a_bled_ops,
					       &props);

	if (IS_ERR(pchip->bled)) {
		rc = PTR_ERR(pchip->bled);
		pr_debug("[SGM37604A] failed to register backlight device, rc=%d\n", rc);
		return rc;
	}
