...
Bootstrap images prompt on the first local tty login for a username and password, create that user locally, run `nixos-rebuild boot` with `USER=<username>`, set the password, and ask for a reboot.
